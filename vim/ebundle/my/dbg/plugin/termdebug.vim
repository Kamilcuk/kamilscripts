...
func TermDebugSendCommandNoContinue(cmd)
	if s:way == 'prompt'
		call ch_sendraw(s:gdb_channel, a:cmd . "\n")
	else
		call term_sendkeys(s:gdbbuf, a:cmd . "\r")
...
func TermDebugSendCommandComm(cmd)
...
func s:InstallCommands()
  ...
  command -nargs=? Clear call s:ClearBreakpoint(<q-args>)
  command -nargs=? ToggleBreak call s:ToggleBreakpoint(<q-args>)
...
func s:SetBreakpoint(at)
  ...
  let bploc = empty(a:at) ? fnameescape(expand('%:p')) . ':' . line('.') : a:at
  call s:SendCommand('-break-insert ' . bploc)
  if do_continue
...
func s:ToggleBreakpoint(at)
	let bploc = empty(a:at) ? fnameescape(expand('%:p')) . ':' . line('.') : a:at
	if has_key(s:breakpoint_locations, bploc)
		call s:ClearBreakpoint(bploc)
	else
		call s:SetBreakpoint(bploc)
	endif
endfunc
